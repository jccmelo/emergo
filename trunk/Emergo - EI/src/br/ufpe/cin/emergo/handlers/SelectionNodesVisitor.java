...
public class SelectionNodesVisitor extends ASTVisitor {
	...
	private Set<ASTNode> nodes = new LinkedHashSet<ASTNode>();
	...
	/**
	 * Populates the {@link #nodes} Set with the ASTNodes. Use {@link #getNodes()} to retrive the nodes after accepting
	 * this visitor to an ASTNode
	 * 
	 * @see org.eclipse.jdt.core.dom.ASTVisitor#preVisit(org.eclipse.jdt.core.dom.ASTNode)
	 */
	public void preVisit(ASTNode node) {
		super.preVisit(node);
		if (node.getStartPosition() >= textSelection.getOffset() && (node.getStartPosition() + node.getLength()) <= textSelection.getOffset() + textSelection.getLength()) {
			nodes.add(node);
		}
	}
}
